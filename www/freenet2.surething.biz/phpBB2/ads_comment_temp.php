...
else
{
	...
	// --------------------------------
	// Send an email to each comment poster
	// --------------------------------

	$user_id_array[] = $comment_user_id;
	...
	while ($row = $db->sql_fetchrow($result)) 
	{
		...
		if ( in_array($comment_user_id, $user_id_array ) )
		{
			// Ignore!
		}
		else
		{
			...
			// Get recipients details from phpBB
			$profiledata = get_userdata($comment_user_id); 
			$recip_email = $profiledata['user_email']; 
